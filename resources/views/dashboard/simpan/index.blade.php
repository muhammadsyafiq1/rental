    ...
    <table id="tb_simoan" class="table table-striped">
      ...
      <tbody>
        @foreach ($simpan as $simpan)
              <tr class="bg-dark">
                  <td>
                    <img src="{{ Storage::url($simpan->car->gallery->first()->foto ?? 'not found') }}" style="width: 140px;">
                  </td>
                  <td>{{$simpan->car->nama_mobil}}</td>
                  <td>
                    {{$simpan->car->lepas_kunci == 0 ? 'Tidak, (Biaya supir berlaku)' : 'Iya'}} <br>
                    <small class="text-warning">Rp .{{$simpan->car->biaya_supir }}</small>
                  </td>
                  ...
                  <td>
                    ...
                    <a href="{{route('detail',$simpan->id)}}" class="btn btn-sm btn-warning"> Lihat Mobil</a>
                  </td>
              </tr>
          @endforeach
      </tbody>
